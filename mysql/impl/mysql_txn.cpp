...
void MysqlTxn::addQueryManagerId(bool isRealIndex, char* pDesignatorName, void* pHandler,
        char* pKey, unsigned int aKenLength, unsigned short aQueryMgrId,
        unsigned char mysqlInterfaceDebugLevel) {
...
unsigned short MysqlTxn::findQueryManagerId(char* aDesignatorName, void* pHandler, char* aKey,
	...
	for (int i = 0; i < QueryManagerIdCount_; ++i) {
		QueryManagerInfo* pQueryManagerInfo = (QueryManagerInfo*) SDBArrayGetPtr(
		        pQueryManagerInfoArray_, i);

		// We found the query manger id if both the table handler object and designator match. 
		if ((pQueryManagerInfo->pHandler_ == pHandler) && SDBUtilCompareStrings(aDesignatorName,
		        pQueryManagerInfo->pDesignatorName_, true)) {
...
void MysqlTxn::addLockTableName(char* pLockTableName) {
	...
	for (i = 1; i <= maxPosition; ++i) { // element starts at position 1
		...
		if (SDBUtilCompareStrings(pCurrTableName, pLockTableName, true, (unsigned short) strlen(
		        pLockTableName)))
			break;
	}

	if (i > maxPosition) { // add lock table name if not found.
		// get unused location in the LockTables array
		unsigned short position = SDBArrayGetUnUsedPosition(pLockTablesArray_);
